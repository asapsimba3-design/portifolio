...
function setupEventListeners() {
  ...
  document.getElementById('show-signup').addEventListener('click', (e) => {
    ...
    document.getElementById('auth-title').textContent = 'Admin Sign Up';
    ...
    try {
      ...
      if (error) {
        ...
        if (data.user && data.user.identities && data.user.identities.length === 0) {
          ...
          errorEl.textContent = 'Account created! Please check your email to confirm your account, then login.';
        } else {
          // Auto-login successful
          currentUser = data.user;
          console.log('Auto-login successful, showing admin panel...');
          showAdminPanel();
          loadData();
        }
      }
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (data && data.user) {
        ...
        errorEl.textContent = 'Login successful! Loading admin panel...';
        ...
        errorEl.style.color = '#e74c3c';
        errorEl.textContent = 'Login failed. No user data received.';
      }
    } catch (err) {
      console.error('Unexpected error:', err);
      errorEl.textContent = 'An unexpected error occurred. Please check the console.';
